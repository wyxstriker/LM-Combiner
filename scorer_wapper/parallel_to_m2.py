...
def annotate(args, line):
    ...
    for idx, target in enumerate(sent_list[1:]):
        try:
            ...
            # raise Exception
            source_tokenized = sentence_to_tokenized[source]
            target_tokenized = source_tokenized
            out, cors = annotator(source_tokenized, target_tokenized, idx)
            if idx == 0:
                output_str = "".join(out[:-1])
            else:
...
def main(args):
    ...
    with open(args.output, "w", encoding="utf-8") as f:
        ...
        for line in tqdm(lines):
            ret = annotate(args, line)
            # f.write(ret)
            # f.write("\n") 
            # new
            ret = ret.strip().split('\n')
            input_text = ''.join(ret[0].split(' ')[1:])
            output_text = ''.join(ret[1].split(' ')[1:])
            label_list = []
            for item in ret[2:]:
                if item[0] != 'A':
                    break
                item = item.split('|||')
                error_index = [' '.join(list(num)) for num in item[0].split(' ')[1:]]
                type_map = {'S': '[S]', 'W': '[W]', 'R': '[R]', 'M': '[M]', 'noop': '[NOOP]'}
                error_type = type_map.get(item[1])
                error_content = ''.join(item[2].split(' '))
                if error_content == '-NONE-':
                    error_content = '[NONE]'
                error_info = '[INNER]'.join(error_index) + '[INNER]' + error_type + '[INNER]' + error_content
                # error_info = error_type + '[INNER]' + error_content
                label_list.append(error_info)
            insert_item = {}
            insert_item['input'] = input_text
            insert_item['cot'] = '[SPLIT]'.join(label_list)
            insert_item['output'] = input_text if output_text=='没有错误' else output_text
            res_item.append(insert_item)
        import json
        json.dump(res_item, f, ensure_ascii=False, indent=2)
