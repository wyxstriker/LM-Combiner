...
def main(hyp, ref):
    ...
    hyp_m2 = open(hyp).read().strip().split("\n\n")[args.start:args.end] if args.start is not None and args.end is not None else open(hyp).read().strip().split("\n\n")
    ref_m2 = open(ref).read().strip().split("\n\n")[args.start:args.end] if args.start is not None and args.end is not None else open(ref).read().strip().split("\n\n")
...
class eval_parser:
    ...
    single:bool = False
    multi:bool = False
    multi_hyp_avg:bool = False
    multi_hyp_max:bool = False
    filt:bool = False
    multi_hyp_max:bool = False
    cat:bool = False
...
def print_results(best, best_cats, args):
    ...
    print('{:=^46}'.format(""))
    print("")
    return computeFScore(best["tp"], best["fp"], best["fn"], args.beta)
